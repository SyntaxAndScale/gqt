...
- [ ] Update the right pane (task detail) to show each of the task's Tags, Title, Assignee, Task Creation Date, Due Date (from the API's 'rawDate'), Repeat information (if repeating is not false), then Notes. For the tags, prepend the tag name with a pound sign ('#') and use a background color of dark yellow and white text. The tags do not need to each be on separate lines - instead they should all appear on one line, word-wrapping when necessary. Similarly, the Created Date, Due Date and Reapting information should also be on one line unless it needs to be word-wrapped.
